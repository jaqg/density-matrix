...
program main
    ...
    call read_H2_matrix(h2fn, .true., H2MO)  ! two-electron density matrix

    ! Read CAS info from one of the files
    call read_info(d1fn, norbt, nisht, nasht, nvirt, Nactel, Nelec)
    if (tstdbg) then
        write(ouf,'(a,i0)') 'Total number of orbitals, Norbt = ', norbt
        write(ouf,'(a,i0)') 'Number of inactive orbitals, Nisht = ', nisht
        write(ouf,'(a,i0)') 'Number of active orbitals, Nasht = ', nasht
        write(ouf,'(a,i0)') 'Number of virtual orbitals, Nvirt = ', nvirt
        write(ouf,'(a,i0)') 'Number of active electrons, Nactel = ', nactel
        write(ouf,'(a,i0)') 'Total number of electrons, Nelec = ', nelec
    end if
    ...
    ! Symmetrize the 2-RDM
    call symmetrize_D2(D2MO, D2MOsym)
    D2MO = D2MOsym
    ...
    call checknorm_D2(D2MO, Nelec, fthres, ouf)
    ...
    call H1D1_energy(D1MO/2.d0, H1MO, EH1D1)  ! test: deberia dar lo mismo
    ...
    ! TODO: usar el bueno
    ! call Eee_D2(D2MOsym, H2MO, EH2D2)  ! bueno
    call Eee_D2(D2MO/2.d0, H2MO, EH2D2)  ! test
    ! call Eee_D2(D2MO, H2MO, EH2D2)  ! test: deberia dar lo mismo
    ...
    ! Check norm of each approximated 2-RDM
    write(ouf,'(a)') 'D2LS';call checknorm_D2(D2LSSO, Nelec, fthres, ouf);write(ouf,*)
    write(ouf,'(a)') 'D2BBC1';call checknorm_D2(D2BBC1SO, Nelec, fthres, ouf);write(ouf,*)
    write(ouf,'(a)') 'D2BBC2';call checknorm_D2(D2BBC2SO, Nelec, fthres, ouf);write(ouf,*)
    write(ouf,'(a)') 'D2BBC3';call checknorm_D2(D2BBC3SO, Nelec, fthres, ouf);write(ouf,*)
    write(ouf,'(a)') 'D2BBC3M';call checknorm_D2(D2BBC3MSO, Nelec, fthres, ouf);write(ouf,*)

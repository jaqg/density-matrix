...
module io 
    ...
    subroutine read_info(fn, norbt, nisht, nasht, nvirt, Nactel, Nelec)
        implicit none
        character(len=*), intent(in) :: fn
        integer, intent(out) :: norbt, nisht, nasht, nvirt, Nactel, Nelec
        !
        integer :: nlu, ios 

        ! Open file
        open(newunit=nlu, file=trim(fn), iostat=ios)
        if (ios /= 0) stop "read_info: Error opening file trim(fn)"

        ! Read dimensions of Amat
        read(nlu,*) 
        read(nlu,*) norbt, nisht, nasht, nvirt, Nactel, Nelec
        !
        return
    ...
    subroutine read_H2_matrix(fn, is_triangular, Amat)
        !
        ! fn: matrix input file name
        ! Amat: two-electron integrals
        !
        implicit none
        character(len=*), intent(in) :: fn
        logical :: is_triangular
        ...
        ! Open file
        open(newunit=nlu, file=trim(fn), iostat=ios)
        if (ios /= 0) stop "read_H2_matrix: Error opening file trim(fn)"
        ...
        ! Allocate Amat
        allocate(Amat(n,n,n,n), stat=ios)
        if (ios /= 0) stop "read_H2_matrix: Error in allocation of Amat"
        Amat = 0.d0
        ...
        do
            read(nlu,*,iostat=ios) kind, lind, iind, jind, xint
            if (ios /= 0) stop 'read_H2_matrix: Error reading file.'
            if (kind.eq.0 .or. lind.eq.0 .or. iind.eq.0 .or. jind.eq.0) exit
        ...
        ! Close file
        close(unit=nlu, iostat=ios, status="keep")
        if (ios /= 0) stop "read_H2_matrix: Error closing file unit nlu"
        
        !
        return
    end subroutine read_H2_matrix 
